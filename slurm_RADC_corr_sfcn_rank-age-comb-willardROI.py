...
#     random_state=RANDOM_STATE,
# )
# train_x, val_x, train_y, val_y = train_test_split(X.reshape(1,-1)[0], y, test_size = 0.2, random_state=RANDOM_STATE, stratify=y)
train_x, val_x, train_y, val_y = train_test_split(X, y, test_size = 0.2, random_state=RANDOM_STATE, stratify=y)
...
class SFCN(nn.Module):
    def __init__(self, channel_number1=[32, 64, 128, 256, 64], channel_number2=[32, 64, 128, 256, 256, 64], output_dim=31, dropout=True):
...
#                 f.write('%s\n' % s)
    ...
    if valid_mae < best_mae:
        ...
        ########## SAVE MODEL #############
        torch.save(model.state_dict(), os.path.join(inPATH, f'best_model_sfcn_comb_seed_{RANDOM_STATE}.pt'))
...
all_pred = []
all_probas = []
model.load_state_dict(torch.load(os.path.join(inPATH, f'best_model_sfcn_comb_seed_{RANDOM_STATE}.pt')))
with torch.set_grad_enabled(False):
