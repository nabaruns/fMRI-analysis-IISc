#!/bin/sh
#SBATCH --job-name=qsi_recon    # Job name
#SBATCH --array=1-1%1
...
mkdir -p $BIDS_DIR/$subject && mkdir -p $OUT_DIR/$subject && mkdir -p $OUT_DIR/recon_$subject && mkdir -p $WORK_DIR/$subject
...
# 	--work $WORK_DIR/$subject/
# 	--user $(id -u $USER):$(id -g $USER) \
#     --bids_dir $BIDS_DIR/$subject/ \
#     --recon_input $OUT_DIR/$subject/ \
#     --recon_spec mrtrix_singleshell_ss3t \
#     --output_dir $OUT_DIR/recon_$subject/ \
#     --analysis_level participant \
#     --fs-license-file $HOME/license.txt"

cmd="docker run --init --name $SLURM_JOB_ID --user $(id -u $USER):$(id -g $USER) --rm \
	-v $BIDS_DIR/$subject/:/data:ro \
	-v $OUT_DIR/$subject/:/qsiprep-output:ro \
	-v $OUT_DIR/recon_$subject/:/out \
	-v $HOME/license.txt:/opt/freesurfer/license.txt:ro \
