...
for d in /media/nabarun/TATA_MRI_Data_RAW/TATA_MRI_*; do
	for subj in $( ls $d ); do
		...
		BIDS_DIR="$HOME/TATA/BIDS_rest/$sub_dir"
		...
		funcinit=$( ls -S $BIDS_DIR/tmp_dcm2bids/helper/ | grep RESTING | head -1 | sed "s/[_\.\-][0-9A-Za-z]*//g" )
		echo '{"descriptions":[{"dataType": "anat","modalityLabel":"T1w","criteria": {"SidecarFilename": "*TA_9.14*"}},{"dataType": "func","modalityLabel":"bold","customLabels":"task-rest","criteria": {"SidecarFilename": "'$funcinit'**RESTING_*"}}]}' > $BIDS_DIR/code/config.json
		...
		rm -rf $BIDS_DIR/tmp_dcm2bids
		cp -r $BIDS_DIR $BASE_DIR/BIDS_rest/
		rm -rf $BIDS_DIR
