...
infile = open(os.path.join(inPATH, "GM_all_nan"),'rb')
...
# infile = open('Willard499_all_nan','rb')
...
for fold, (train_index, val_index) in enumerate(kf.split(train_x)): 
    ...
    for epoch in range(num_epochs):
        ...
        if valid_mae < best_mae:
            ...
            torch.save(model.state_dict(), os.path.join(inPATH, f'best_model_rank_gm_fold_{fold}.pt'))
    ...
    model.load_state_dict(torch.load(os.path.join(inPATH, f'best_model_rank_gm_fold_{fold}.pt')))
#     model.eval()
#     with torch.set_grad_enabled(False):
...
#     print(s)
    results[fold] = (best_mae, best_rmse, best_epoch)
...
all_pred = []
all_probas = []
model.load_state_dict(torch.load(os.path.join(inPATH, 'best_model_rank_gm_fold_0.pt')))
model.eval()
with torch.set_grad_enabled(False):
    test_mae, test_mse = compute_mae_and_mse(model, test_loader,
                                               device=DEVICE)
...
print(np.abs(np.array(all_out['majority']) - test_y).sum()/len(test_y))


# In[49]:


# plt.scatter(test_y[:]+64, np.array(all_out['majority'])[:]+64)
