...
class Iterator(object):
    ...
    def __init__(self, n, batch_size, frames_per_step, shuffle, seed):
        ...
        self.index_generator = self._flow_index(n, batch_size, 1, shuffle, seed)
...
class DirectoryIterator(Iterator):
    ...
    def next(self):
        ...
        # The transformation of images is not under thread lock
        # so it can be done in parallel
        bs = int(self.image_shape[0]/self.frames_per_step)
        # batch_x = np.zeros((current_batch_size,)  +(self.frames_per_step,)+       
        batch_x = np.zeros((bs,)  +(self.frames_per_step,)+       
                           self.image_shape[1:], dtype=K.floatx())   # # my addition of +(1,)
        ...
        for i in range(bs):
            # batch_x[i,kk] = x
            st = i*self.frames_per_step
            en = min(st+self.frames_per_step, self.image_shape[0])
            batch_x[i] = x[st:en,:,:,:,:]
